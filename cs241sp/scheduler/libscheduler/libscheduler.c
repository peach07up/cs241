...
{
    // We'll use this function to update the remaining time
    // We'll keep track of the last time we checked with last_check_time
    // And subtract the difference from last_check_time and the current time
    // If last_check_time is -1, it means this program has not been running
    // So set last_check_time to now to start counting
    // We only loop through the programs that are currently on the processor cores
    // running
    for(int i = 0; i < num_cores; i++)
    {
        ...
        // This job just started this run, start logging the time starting from now
...
int scheduler_new_job(int job_number, int time, int running_time,
    ...
    if(scheme == PSJF)
    {
        ...
        if(core >= 0)
        {
            ...
            // Set the last run time to now to keep track of it
            cores[core].job->last_run_time = time;
            // If remaining time = total time, this means we just switched
            // to this job, and are instantly preempting it
            // So it never actually got to run, so reset it's response time
            if(cores[core].job->remaining_time == cores[core].job->total_time)
            {
                cores[core].job->response_time = -1;
            }
