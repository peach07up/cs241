...
int get_end_timestamp(int e, StatsInfo *info) {
    ...
    for (i = 0; i < info->wearable_size_; i++) {
        Wearable *cur = info->wearables_[i];
        for (k = 0; cur->data_size_; k++) {
...
void *stats_thread(void *arg) {
    ...
    for (i = 0; i < sample_size * 3;
        ...
        set_alarm(get_end_timestamp(t2, info)); // set an alarm to fire after we
        // should have received our data
...
int main(int argc, const char *argv[]) {
    ...
    // create wearables
    Wearable **wearables = get_wearables(argv[3], &count, &sample_times,
                                         &sample_count, &latest_launch);
    ...
    for (i = 0; i < sample_count * 3; i += 3) {
        ...
        queue_gather(&complete_data_set, t1, t2, get_type1_selector, &size);
        print_results(TYPE1, results, size);
        free(results);
        
        results =
        queue_gather(&complete_data_set, t1, t2, get_type2_selector, &size);
        print_results(TYPE2, results, size);
        free(results);
        
        results =
        queue_gather(&complete_data_set, t1, t2, get_type3_selector, &size);
