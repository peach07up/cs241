...
int parmake(int argc, char **argv) {
    ...
    {
        struct stat buf;
        if(stat("makefile", &buf) == 0)
        {
            makefile = "makefile";
        }
        else if(stat("Makefile", &buf) == 0)
        {
            makefile = "Makefile";
        }
        else
...
target_t *new_target(char *target)
{
    ...
    tar->run = NOT_RUN;
    tar->target = malloc(sizeof(char) * (strlen(target) + 1));
    strcpy(tar->target, target);
    tar->is_file = 0;
...
void parsed_new_command(char *target, char *command)
{
    ...
    strcpy(cmd, command);
    queue_enqueue(&(tar->commands), cmd);
}
